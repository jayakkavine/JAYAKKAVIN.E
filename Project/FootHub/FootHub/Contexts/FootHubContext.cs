...
public partial class FootHubContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.HasKey(e => e.ProductLink).HasName("product_link_pri");

            entity.ToTable("ProductLinkTable");

            entity.HasIndex(e => new { e.PId, e.OId, e.TId, e.BId, e.CId }, "productLink_unique").IsUnique();

            entity.Property(e => e.ProductLink).HasColumnName("product_link");
            entity.Property(e => e.BId).HasColumnName("b_id");
            entity.Property(e => e.CId).HasColumnName("c_id");
            entity.Property(e => e.OId).HasColumnName("o_id");
            entity.Property(e => e.PId).HasColumnName("p_id");
            ...
            entity.HasOne(d => d.BIdNavigation).WithMany(p => p.ProductLinkTables)
                .HasForeignKey(d => d.BId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("brand_fore");

            entity.HasOne(d => d.CIdNavigation).WithMany(p => p.ProductLinkTables)
                .HasForeignKey(d => d.CId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("category_fore");

            entity.HasOne(d => d.OIdNavigation).WithMany(p => p.ProductLinkTables)
                .HasForeignKey(d => d.OId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("ocassion_fore");

            entity.HasOne(d => d.PIdNavigation).WithMany(p => p.ProductLinkTables)
                .HasForeignKey(d => d.PId)
                .OnDelete(DeleteBehavior.ClientSetNull)
                .HasConstraintName("product_fore");

            entity.HasOne(d => d.TIdNavigation).WithMany(p => p.ProductLinkTables)
